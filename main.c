...
void insereChaves(){
	...
	c = insereValor(c, "TabelaOrigem", "Cargo");
	c = insereValor(c, "ID", "4");
	c = insereValor(c, "Tipo", "P");
	c = insereValor(c, "TabelaDestino", "");
	c = insereValor(c, "CampoOrigem", "CodCarg");
	c = insereValor(c, "CampoDestino", "");
	
	c = insereValor(c, "TabelaOrigem", "Funcionario");
	c = insereValor(c, "ID", "2");
	c = insereValor(c, "Tipo", "P");
	c = insereValor(c, "TabelaDestino", "");
	c = insereValor(c, "CampoOrigem", "CodFunc");
	c = insereValor(c, "CampoDestino", "");
	
	c = insereValor(c, "TabelaOrigem", "Funcionario");
	c = insereValor(c, "ID", "5");
	c = insereValor(c, "Tipo", "F");
	c = insereValor(c, "TabelaDestino", "Cargo");
	c = insereValor(c, "CampoOrigem", "CodCarg");
	c = insereValor(c, "CampoDestino", "CodCarg");

	erro = finalizaInsert("BD_Chaves", c);

	if(erro == ERRO_VIOLACAO_PK){
		printf("\nErro %d: A tabela ja possui chave primaria(PK) !\n",erro);
...
int consultar(){
	...
	if(!existe_arquivo(arquivo_dados)){
		printf("\nErro: Nao existem tuplas cadastradas nesta tabela! (Tabela Vazia)\n");
		return 0;
	}

	if(strcmp(tabela, "BD_Chaves") == 0)
		printf("\nVoce selecionou Consultar as Chaves(PK/FK)!\nDigite a tabela que deseja consultar as chaves:\n");
	else
...
int criar_tabela(char nomeTabela[], int tipo_tabela){
	...
	if(t == ERRO_NOME_TABELA_INVALIDO){
		printf("Erro: na função iniciaTabela(). A tabela %s já existe.\n",nomeTabela);
		return 0;
	}
	...
	switch(tipo_tabela){
		...
		case 1://Funcionarios
			...
			t = adicionaCampo(t, "CodFunc", 'I', (sizeof(int)));
			t = adicionaCampo(t, "Sexo", 'C', (sizeof(char)));
			t = adicionaCampo(t, "Obs", 'S', 40);
			t = adicionaCampo(t, "CodCarg", 'I', (sizeof(double)));
		break;
		
		case 2://Produtos
			t = adicionaCampo(t, "Produto", 'S', 20);
			t = adicionaCampo(t, "CodProd", 'I', (sizeof(int)));
		...
		case 3://Cargos
			...
			t = adicionaCampo(t, "CodCarg", 'I', (sizeof(int)));
	...
	printf("\nTabela %s criada com sucesso!",nomeTabela);
...
void criar_tabelas_padrao(){
	criar_tabela("Funcionario",1);
	criar_tabela("Produto",2);
	criar_tabela("Cargo",3);
	insereChaves();
}
...
int menu(){
	...
	switch(opcao){
		...
		case 2: // Criar tabelas padrões (Produto, Funcionario, Cargo)
